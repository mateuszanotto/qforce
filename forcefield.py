    ...
    def write_frcmod_dihedrals(self, frcmod, terms, j2cal):
        ...
        if len(terms['dihedral/flexible']) > 0:
            ...
            for dihed in terms['dihedral/flexible']:
                ids = dihed.atomids + 1
                c = dihed.equ
                fc = [(1.0*c[0] + 0.5*c[2] + 0.3750*c[4])* j2cal,
                      (1.0*c[1] + 0.75*c[3] + 0.6250*c[5])* j2cal,
                      (0.5*c[2] + 0.5*c[4])* j2cal,
                      (0.25*c[3] + 0.3125*c[5])* j2cal,
                      (0.125*c[4])* j2cal,
                      (0.0625*c[5])* j2cal]
                ...
                for n in range(5, 0, -1):
                    if n % 2 == 1:
                        ...
                        equ = 0.00
                    frcmod.write(f"{ids[0]:<2}-{ids[1]:<2}-{ids[2]:<2}-{ids[3]:<2}")
                    frcmod.write(f" 2 {fc[n]:>15.6f} {equ:>15.2f} -{n+1}\n")

                # Last term
                frcmod.write(f"{ids[0]:<2}-{ids[1]:<2}-{ids[2]:<2}-{ids[3]:<2}")
                frcmod.write(f" 2 {fc[0]:>15.6f} {0.00:>15.2f}  1\n")
